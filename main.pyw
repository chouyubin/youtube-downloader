...
def setup_styles():
    ...
    s.configure("TProgressbar", background=C["green"], troughcolor=C["bg3"], borderwidth=0, thickness=10)
    s.configure("TScrollbar", background=C["bg3"], troughcolor=C["bg"], borderwidth=0, arrowcolor=C["fg2"])
...
class SettingsDialog:
    ...
    def _build(self):
        T = self.T
        m = Frame(self.w, bg=C["bg"]); m.pack(fill=BOTH, expand=True, padx=16, pady=16)
        ...
        if self.app.downloader.gpu_info["type"]:
            ...
            Checkbutton(gpf, text=f"{gpu_name} ({gpu_full})",
                variable=self.app.gpu_enabled_var, bg=C["bg2"], fg=C["purple"],
                selectcolor=C["bg3"], activebackground=C["bg2"], activeforeground=C["purple"],
                font=("Microsoft YaHei",9), relief=FLAT, bd=0, cursor="hand2").pack(side=LEFT, padx=(8,12))
            gcm = OptionMenu(gpf, self.app.gpu_codec_var, "h264","hevc","av1")
            gcm.configure(bg=C["bg3"], fg=C["fg"], font=("Consolas",8), relief=FLAT, bd=0, highlightthickness=0, indicatoron=0, padx=4)
            gcm["menu"].configure(bg=C["bg3"], fg=C["fg"], font=("Consolas",8), relief=FLAT)
            gcm.pack(side=LEFT)
    ...
    def _rs(self):
        ...
        self.app.gpu_enabled_var.set(False); self.app.gpu_codec_var.set("h264")
...
class YouTubeDownloaderApp:
    def __init__(self, root):
        ...
        self.gpu_codec_var = StringVar(value="h264")
    ...
    def _build_ui(self):
        ...
        ff_ok = T("ffmpeg_ok") if self.has_ffmpeg else T("ffmpeg_no")
        Label(h, text=ff_ok, fg=C["green"] if self.has_ffmpeg else C["red"],
            bg=C["bg"], font=("Consolas", 8)).grid(row=0, column=1, sticky="e", padx=(0,2))
        gpu = self.downloader.gpu_info
        gpu_text = f"GPU: {gpu['type'].upper()}" if gpu["type"] else T("gpu_none")
        Label(h, text=gpu_text, fg=C["purple"] if gpu["type"] else C["fg2"],
            bg=C["bg"], font=("Consolas", 8)).grid(row=0, column=2, sticky="e", padx=(0,4))
        sbtn(h, T("settings"), self._open_settings, C["bg3"], fs=9, bd=False, padx=10, pady=3).grid(row=0, column=3, padx=(0,8))
        ...
        btn_lang.grid(row=0, column=4, padx=(0,4))
        ...
        Checkbutton(hf, text=T("multi_thread"), variable=self.multi_thread_var,
        ...
        self._oc(g, T("audio_only"), self.audio_only_var, 1, 0, 1)
        self._oc(g, T("subtitles"), self.subtitle_var, 1, 1, 1)
        self._oc(g, T("playlist"), self.playlist_var, 1, 2, 1)
        self._oc(g, T("torrent"), self.make_torrent_var, 1, 3, 1)
        ...
        c4.grid_rowconfigure(1, weight=1); c4.grid_columnconfigure(0, weight=1)
        ...
        lf = Frame(c4, bg=C["bg2"]); lf.grid(row=1, column=0, sticky="nsew", padx=12, pady=(0,8))
        lf.grid_rowconfigure(0, weight=1); lf.grid_columnconfigure(0, weight=1)
        self.log_text = Text(lf, bg=C["bg"], fg=C["fg"], font=("Consolas",9),
    ...
    def _oc(self, p, t, v, r, c, s=1):
        Checkbutton(p, text=t, variable=v, bg=C["bg2"], fg=C["fg"], selectcolor=C["bg3"],
            activebackground=C["bg2"], activeforeground=C["fg"], font=("Microsoft YaHei",9),
            relief=FLAT, bd=0, cursor="hand2").grid(row=r, column=c, sticky="w", pady=5, columnspan=s)
        ...
        def _do():
            ...
            if "error" in info:
                self.root.after(0, lambda: [
                    self._log(T("parse_error").format(info["error"]), "err"),
    ...
    def _show_info(self, info):
        ...
        if info.get("formats"):
            hts = sorted(set(f["resolution"] for f in info["formats"]), reverse=True)
    ...
    def _on_download(self):
        ...
        self.downloader.set_gpu(self.gpu_enabled_var.get(), self.gpu_codec_var.get())
